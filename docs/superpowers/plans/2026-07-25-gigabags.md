# GigaBags Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship the GigaBags demo path (dossier + receipts + Said-vs-Did + one real FADE swap) by Sun Jul 26, 8:30 AM WEST.
...
- Env vars in `.env.local`: `ZG_API_KEY`, `PINAX_JWT`, `UNISWAP_API_KEY`, `GRAPH_STUDIO_KEY`, `DB_PATH=./gigabags.db`.
...
```bash
cd ~/Documents/RandomClaudeSessions/gigabags
...
export function getDb() {
  if (!db) {
    db = new Database(process.env.DB_PATH ?? "./gigabags.db");
