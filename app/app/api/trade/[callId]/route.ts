...
import { isNetwork, netCfg, resolveToken, type Network } from "@/lib/networks";
...
export async function POST(req: Request, { params }: { params: Promise<{ callId: string }> }) {
  ...
  // network up front. If there's no pool for it here, tell the user
  // immediately (the instant they click) instead of dropping into a swap
  // form for an asset that can't settle on this chain.
  const tokenAddress = resolveToken(network, call.asset_symbol);
  if (!tokenAddress) {
    return NextResponse.json({
      status: "no_pool",
      network,
      asset: call.asset_symbol,
      reason: `No liquidity pool for $${call.asset_symbol} on ${net.label.toLowerCase()}.`,
    });
  }
  // The asset resolves to the quote asset itself (can't swap a token against
  // itself), so report it up front rather than failing downstream. On testnet
  // the quote is USDC, so an $ETH call (→ WETH) is tradeable; a $USDC call is
  // the one that gets blocked here.
  if (tokenAddress.toLowerCase() === net.quoteAsset.toLowerCase()) {
    return NextResponse.json({
      status: "no_pool",
      network,
      asset: call.asset_symbol,
      reason: `$${call.asset_symbol} is the quote asset on ${net.label.toLowerCase()} — no directional pool to trade.`,
    });
  }
  ...
  const signal: CallSignal = {
    ...
    tokenAddress, // network-resolved (Base testnet/mainnet), not the pricing address
