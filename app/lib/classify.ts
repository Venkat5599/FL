/**
 * MIRROR OF THE ENCLAVE CLASSIFIER — copied verbatim from
 *   tee/typescript/src/app/classify.ts
 *
 * Why a copy rather than a shared package: the TEE extension is built and hashed as its
 * own reproducible image, and adding a workspace dependency between it and the Next.js
 * app would put app code inside the attested build. The enclave's dependency graph
 * should contain exactly what it needs to classify a post, and nothing else.
 *
 * What this copy is FOR: an instant local preview, so the UI can show what a post looks
 * like as a signal without waiting on a round trip through the chain and the enclave.
 *
 * What it is NOT: authoritative. The verdict that reaches the tape is the one the TEE
 * signed — this is a prediction of it. They agree because the logic is identical and
 * deterministic (tests/classify-parity.test.ts pins that), but if they ever disagree,
 * the enclave is right and this is a bug.
 *
 * Keep the two files byte-identical below this header.
 */
